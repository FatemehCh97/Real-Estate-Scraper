...
def get_property_item_info(url):
    ...
    if response.status_code == 200:
        ...
        # Attributes
        ## Price information
        price_container = bsobj.find('div',
                                     class_=('SpotlightAttributesPrice_item'
                                             '_iVKUf'))
        ...
        ## Living Space
        living_space_container = bsobj.find('div', class_='SpotlightAttributesLivingSpace_item_daF4o')

        if living_space_container:
            living_space = living_space_container.find('div', class_='SpotlightAttributesLivingSpace_value_OiuVM').text.strip()
            ad_info['Living Space'] = living_space

        # Location
        ## Address
        location_container = bsobj.find('div', 'AddressDetails_addressDetails_wuB1A')
        ...
        # Tip & Services
        services_title = bsobj.find("section",
                                    class_=("DetailPage_serviceBoxes"
                                            "_gwseJ")).find("h2").text.strip()
        service_boxes = bsobj.find_all('div',
                                       class_=('ServiceBoxes_serviceBoxWrapper'
                                               '_dc_xv'))

        tips_dict = {}
        for box in service_boxes:
            box_title = box.find('h5',
                                 class_='ServiceBox_title_nw3Em').text.strip()
            box_description = box.find('p',
                                       class_=('ServiceBox_description'
                                               '_QgvnI')).text.strip()
            box_link = box.find('a')['href']
            box_link_text = box.find('p',
                                     class_='ServiceBox_cta_Hhc1Q').text.strip()
        ...
        # Images
        # img_container = bsobj.find('div', class_='DetailPage_listingImageGalleryWrapper_bHJaJ').find_all('img')
